...
public class Calendar: InstanceEntity {
    ...
    public Date GetDate(DateTime dateTime) {
        if (!TryGetDate(dateTime, out Date date)) {
            throw new Exception($"DateTime {dateTime.Minutes} is equivalent to Date {dateTime.Minutes / MinutesPerDay} which is a greater magnitude than allowed for a date value");
        }
        return date;
    }
    /// <summary>
